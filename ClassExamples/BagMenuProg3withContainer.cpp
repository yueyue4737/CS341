// John Maslanka  BagMenuProg3withContainer.cpp Baglist Demo  10/17/2008
...
// Applications code -- primarily handles proram's menu
int main () {
	...
	if (!cin.eof()) {
		...
		case 'A': case 'a': {
		    ...
		    cin >> udata;
		    if (!cin.eof())
				A.putANode(udata); // put in message instead of using cont.
		    break;
		}
		case 'D': case 'd': { 
