// Author: John Maslanka   BinarySearchTree3G.cpp
...
public:
  ...
  void postInsertionBalanceWrapper();
...
void BST::postInsertionBalanceWrapper() {
	postInsertionBalance(postTree);
	return;
}
...
// overhead. For ease of programming, nodes now keep track of their parent to 
// facilitate the rotating procedure. 
// Note that the AVL technique does not handle the problem of deleting a Node
// from a BST.

void BST::postInsertionBalance(Node* n)  //In this program this method is called by a manual entry  
										 // from the user to balance the tree. Imbalances can be dealt 
										 // with by looking recursively at the parent line of the new 
										 // Node all the way up to the root, as mentioned above.
{
	Node* p;
	...
	if (n != root) //run recursively until we travel up to root
	{
...
int main () {
  ...
  cout << "Add a node, Balance, Display a node, Show sorted list, display Tree, Help, Exit: " << endl;
  while (!cin.eof() && op != 'E' && op != 'e')  // build binary tree in this loop
	...
	switch (op)
	  ...
	  case 'B': case 'b':
		{	if (TREE.getCount())
			{ cout << endl << "Invoking post-Insertion Balancing" << endl; 
			  TREE.postInsertionBalanceWrapper();
			}
		else
			cout << endl << "Tree is empty. No entries found." << endl;
		  break;
	  }
	  case 'S': case 's':
	    {if ( TREE.getCount ())
