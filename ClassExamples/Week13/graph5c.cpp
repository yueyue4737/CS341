...
class Graph {
private:
	bool    edge[MAX][MAX];  // square matrix of individual segments of a trip, not efficient/ col first, row second
  // how to use dynamic array to write the bfs
...
public:
	...
	void add_vertexes(const int); // add vertices dynamically??
	void add_edge(int source, int target);
	bool get_edge(int a, int b) { return edge[a][b]; } // return value: t/f from location in the adjacency matrix
	bool is_edge(int source, int target) const; // main algo stops here
...
void Graph::add_vertexes(const int nr)  // populate the vertices on the map
{
	int x, y;
...
void Graph::tree_level(int x, int level)  // recursive methodto follow trips through all of their segments
{
	int j, i;
	++level;
	++vflag[x];
	for (j = 0; j < size(); ++j)
	{
		if (is_edge(x, j)) // by default, testing for true
		{
...
int main() {
	...
	cin >> nr;
	if (cin.eof() || !cin.good() || nr < 2 || nr > MAX) // adjacency matrix did not allow the dimension of 1*1
	{
